              ...
              (a:when-let ((spec (gethash (list i) (gethash "1Q" specs-hash))))
                ...
                (let* ((gate-info (hardware-object-gate-information qubit)))
                  (a:when-let ((fidelity (gethash "fRO" spec)))
                    (setf (gate-record-fidelity (gethash '("MEASURE" _) gate-info)) fidelity))
                  (a:when-let ((fidelity (gethash "fRO" spec)))
                    (setf (gate-record-fidelity (gethash '("MEASURE" _ _) gate-info)) fidelity))
                  (a:when-let ((fidelity (gethash "f1QRB" spec)))
                    (setf (gate-record-fidelity (gethash '("RX" (#.pi) _) gate-info)) fidelity))
                  (a:when-let ((fidelity (gethash "f1QRB" spec)))
                    (setf (gate-record-fidelity (gethash '("RX" (#.(- pi)) _) gate-info)) fidelity))
                  (a:when-let ((fidelity (gethash "f1QRB" spec)))
                    (setf (gate-record-fidelity (gethash '("RX" (#.(/ pi 2)) _) gate-info)) fidelity))
                  (a:when-let ((fidelity (gethash "f1QRB" spec)))
                    (setf (gate-record-fidelity (gethash '("RX" (#.(/ pi -2)) _) gate-info)) fidelity))))))
          ...
          :do (setf (gethash "specs" (hardware-object-misc-data link))
              ...
              (let* ((gate-info (hardware-object-gate-information link)))
                (a:when-let ((fidelity (gethash "fCZ" spec)))
                  (setf (gate-record-fidelity (gethash '("CZ" () _ _) gate-info)) fidelity))
                (a:when-let ((fidelity (gethash "fISWAP" spec)))
                  (setf (gate-record-fidelity (gethash '("ISWAP" () _ _) gate-info)) fidelity))
                (a:when-let ((fidelity (gethash "fCPHASE" spec)))
                  (setf (gate-record-fidelity (gethash '("CPHASE" (_) _ _) gate-info)) fidelity))
                (a:when-let ((fidelity (gethash "fPISWAP" spec)))
                  (setf (gate-record-fidelity (gethash '("PISWAP" (_) _ _) gate-info)) fidelity))))))
