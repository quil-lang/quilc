...
"))))
    (flet ((num-instrs (hw)
             (hash-table-count (gethash hw schedule))))
...
"))))
    ...
    (assert (= 2 (hash-table-count schedule)))
    (assert (= 1 (hash-table-count (gethash "q0_ro_tx" schedule))))
    (assert (= 1 (hash-table-count (gethash "q0_ro_rx" schedule))))))
